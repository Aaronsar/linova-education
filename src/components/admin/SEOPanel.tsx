  ...
  return (
    <div className="flex items-start gap-2 py-1.5">
      <div className={`mt-0.5 w-4 h-4 rounded-full flex items-center justify-center shrink-0 ${
        check.passed ? 'bg-green-100' : 'bg-red-50'
      }`}>
        {check.passed ? (
      ...
      <div className="flex-1 min-w-0">
        <div className={`text-xs font-medium ${check.passed ? 'text-gray-700' : 'text-gray-600'}`}>
          {check.label}
        </div>
...
export default function SEOPanel({ title, metaTitle, metaDescription, slug, focusKeyword, sections, onImprove, improving }: Props) {
      ...
      <div className="p-5 bg-gray-50 border-b border-gray-200">
        ...
        {failedCount > 0 && (
          <div className="mt-3 text-center text-xs text-gray-500">
            {failedCount} point(s) à améliorer
          </div>
        )}
      </div>
      ...
      {onImprove && (
        <div className="p-4 border-t border-gray-200 space-y-3">
          ...
          <div className="grid grid-cols-1 gap-2">
            ...
            >
              {improving === 'seo' ? (
                <><svg className="animate-spin h-3 w-3" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" /><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z" /></svg>Optimisation SEO...</>
              ) : '🔍 Améliorer SEO → 80+'}
            </button>
            ...
            >
              {improving === 'geo' ? (
                <><svg className="animate-spin h-3 w-3" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" /><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z" /></svg>Optimisation GEO...</>
              ) : '🤖 Améliorer GEO → 80+'}
            </button>

            {/* Maillage interne — nouveau */}
            <button
            ...
            >
              {improving === 'maillage' ? (
                <><svg className="animate-spin h-3 w-3" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" /><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z" /></svg>Maillage en cours...</>
            ...
            >
              {improving === 'humanize' ? (
                <><svg className="animate-spin h-3 w-3" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" /><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z" /></svg>Humanisation...</>
              ) : '✍️ Humaniser le texte'}
            </button>
          </div>
